...
export default function MainLayout() {
  ...
  return (
    <div className="flex h-screen bg-[#FAFAFA] dark:bg-[#121212]">
      <aside
        className={clsx(
          'fixed inset-y-0 left-0 z-40 transform bg-white text-gray-700 dark:bg-[#1A1A1A] dark:text-gray-300 border-r border-gray-100 dark:border-white/5 transition-all lg:relative lg:translate-x-0',
          sidebarOpen ? 'translate-x-0' : '-translate-x-full',
          collapsed ? 'w-16' : 'w-64',
        )}
      >
        <div className={clsx('flex h-16 items-center border-b border-gray-100 dark:border-white/5', collapsed ? 'justify-center px-0' : 'gap-3 px-6')}>
          <button
            onClick={() => setCollapsed(!collapsed)}
            className="hidden lg:flex rounded-lg p-2 text-gray-400 hover:bg-cara-50 hover:text-cara-700 transition-colors dark:text-gray-500 dark:hover:bg-neutral-800 dark:hover:text-cara-400"
          ...
          {!collapsed && (
            <>
              <svg viewBox="0 0 54 54" className="h-9 w-9 shrink-0 fill-cara-700 dark:fill-cara-500" xmlns="http://www.w3.org/2000/svg">
              ...
              <div>
                <h1 className="text-base font-extrabold tracking-wide text-cara-700 dark:text-cara-400">CARA</h1>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 leading-tight">IUPA</p>
              </div>
            </>
          )}
          <button onClick={() => setSidebarOpen(false)} className={clsx('text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300', collapsed ? 'lg:hidden' : 'lg:hidden ml-auto')}>
            <X className="h-5 w-5" />
          </button>
        </div>

        <nav className="mt-4 space-y-1 px-3 overflow-y-auto max-h-[calc(100vh-11rem)]">
          {visibleItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  clsx(
                    'flex items-center rounded-xl px-3 py-2 text-sm font-medium transition-colors relative',
                    collapsed ? 'justify-center' : 'gap-3',
                    isActive
                      ? 'bg-cara-500 text-white shadow-sm'
                      : 'text-gray-500 hover:bg-cara-50 hover:text-cara-700 dark:text-gray-400 dark:hover:bg-neutral-800 dark:hover:text-cara-400',
                  )
                }
        ...
        <div className="absolute bottom-0 left-0 right-0 border-t border-gray-100 dark:border-white/5 p-4">
          <div className={clsx('flex items-center', collapsed ? 'flex-col gap-3' : 'justify-between')}>
            {!collapsed && (
              ...
              >
                ...
                <p className="text-cara-600 dark:text-cara-400 text-xs">{user?.role}</p>
              </button>
            )}
            <div className={clsx('flex items-center gap-1', collapsed && 'flex-col')}>
              <button
                onClick={toggleTheme}
                className="rounded-lg p-2 text-gray-400 hover:bg-cara-50 hover:text-cara-700 dark:text-gray-500 dark:hover:bg-neutral-800 dark:hover:text-cara-400 transition-colors"
              ...
              <button
                ...
                className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600 dark:text-gray-500 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors"
                title="Cerrar sesión"
              >
